// from https://github.com/ravendb/ravendb/blob/aa8f9fd355b215c53d7e33788cff426461d33715/Raven.Voron/Voron/Util/Crc.cs
...
{
    public unsafe class CrcImproved
    {
                ...
                fixed (uint* t3 = Table_3)
                {
                    ...
                    while ((endPtr - dataPtr) >= 16)
                    {
                            ...
                            crc = *(t3 + (c & 0xff)) ^
                                  ...
                                  *(t1 + ((c >> 16) & 0xff)) ^
                                  *(t0 + (c >> 24));
                            uint32Value = *(dataPtr + 4) | (uint)*(dataPtr + 1 + 4) << 8 | (uint)*(dataPtr + 2 + 4) << 16 | (uint)*(dataPtr + 3 + 4) << 24;
                            c = crc ^ uint32Value;
                            crc = *(t3 + (c & 0xff)) ^
                                  *(t2 + ((c >> 8) & 0xff)) ^
                                  *(t1 + ((c >> 16) & 0xff)) ^
                                  *(t0 + (c >> 24));
                            uint32Value = *(dataPtr + 8) | (uint)*(dataPtr + 1 + 8) << 8 | (uint)*(dataPtr + 2 + 8) << 16 | (uint)*(dataPtr + 3 + 8) << 24;
                            c = crc ^ uint32Value;
                            crc = *(t3 + (c & 0xff)) ^
                                  *(t2 + ((c >> 8) & 0xff)) ^
                                  *(t1 + ((c >> 16) & 0xff)) ^
                                  *(t0 + (c >> 24));
                            uint32Value = *(dataPtr + 12) | (uint)*(dataPtr + 1 + 12) << 8 | (uint)*(dataPtr + 2 + 12) << 16 | (uint)*(dataPtr + 3 + 12) << 24;
                            c = crc ^ uint32Value;
                            crc = *(t3 + (c & 0xff)) ^
                                  *(t2 + ((c >> 8) & 0xff)) ^
                                  *(t1 + ((c >> 16) & 0xff)) ^
